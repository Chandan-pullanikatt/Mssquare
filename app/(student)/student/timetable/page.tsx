...
export default function StudentTimetablePage() {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">Loading your class schedule...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div>
        <h1 className="text-3xl font-extrabold text-gray-900 mb-2 font-heading">My Timetable</h1>
        <p className="text-gray-500 font-medium">View your upcoming classes and scheduled sessions.</p>
      </div>
              ...
              <div className="space-y-3">
                {dayClasses.length > 0 ? (
                  dayClasses.map((item, i) => (
                    <div 
                      key={i} 
                      className="group bg-white p-4 rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:border-violet-100 transition-all duration-300 relative overflow-hidden"
                    >
                      <div className="absolute top-0 left-0 w-1 h-full bg-[#8b5cf6] opacity-0 group-hover:opacity-100 transition-opacity" />
                      
                      <div className="space-y-3">
                        <div>
                          <p className="text-[10px] font-black text-[#8b5cf6] uppercase tracking-tighter mb-1 truncate">
                            {item.course?.title}
                          </p>
      ...
      {timetables.length === 0 && (
        <div className="py-20 text-center bg-white border border-gray-100 rounded-[2.5rem] shadow-sm">
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-1">No Timetable Found</h3>
          <p className="text-gray-500 text-sm max-w-xs mx-auto">
            You don't have any classes scheduled yet. Keep an eye out for updates from your instructors!
          </p>
        </div>
      )}
    </div>
