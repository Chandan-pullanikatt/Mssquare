...
export function CTA() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/apply/match", {
        method: "POST",
        ...
        <div className="relative z-10 max-w-[680px] mx-auto">
          ...
          <form className="rev flex flex-col sm:flex-row gap-4 max-w-[500px] mx-auto mb-8" onSubmit={handleSubmit}>
            ...
            >
              ...
              ) : showSuccess ? "Done!" : "Get Started"}
              {!isSubmitting && !showSuccess && <ArrowRight size={18} strokeWidth={2.5} />}
            </button>
          </form>

          <p className="rev text-[0.85rem] font-bold" style={{ color: '#94A3B8' }}>
            {showSuccess ? "Success! Check your inbox for the welcome guide." : data.footer}
          </p>
        </div>
      </section>
    </>
